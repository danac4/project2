...
double extract_eps(PyObject *list){
    PyObject *tmp1;
    double result;
    tmp1 = PyList_GetItem(list, 0);
    result = PyFloat_AsDouble(tmp1);
    return result;
}
...
static PyObject* fit(PyObject *self, PyObject *args) {
    ...
    Point *points;
    Cluster *clusters;
    if (!PyArg_ParseTuple(args, "iiiiOOO", &k, &n, &dim, &max_iter, &epsilon, &centroids_list, &points_list)) {
        return NULL;
    }
    eps = extract_eps(epsilon);
    points = allocate_mem((int)dim, (int)n);
    create_matrix(points_list, points,(int)dim, (int)n);
    clusters = createClusters((int)dim, centroids_list, (int)k);
